...
- 캐시 태그 taxonomy를 명시한다: per-user 캐시와 shared 캐시를 태그로 분리하고 무효화 소유자를 하나로 둔다.
  - auth 스코프 데이터를 shared로 캐싱하면 사용자 A가 B의 데이터를 본다.
- Next 16 캐시 기본값을 전제한다: `fetch`와 GET route handler는 기본 uncached다.
  - 세그먼트 캐싱은 `use cache` + `cacheTag`/`cacheLife`로 하되 `cacheComponents` 플래그가 필요하다(기본 off).
- 무효화는 `revalidateTag(tag, profile)`(프로파일 인자 필수)·`revalidatePath`로 한다. 단일 인자 `revalidateTag(tag)`는 Next 16에서 deprecated다.
- 사용자가 자기 변경을 즉시 봐야 하면 Server Action에서 `updateTag`를 쓴다(read-your-writes, 즉시 만료·같은 요청 refresh).
...
- 낙관적 업데이트가 필요할 때만 서버상태를 클라로 이관한다: 반영 → Server Action → 실패 시 롤백을 명시적으로 구현한다. 성공 후 `updateTag`로 서버 진실과 재동기화한다(위 캐시).
