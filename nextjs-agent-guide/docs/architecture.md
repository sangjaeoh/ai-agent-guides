...
- FSD `widgets`·`pages` 레이어를 두지 않는다.
  - `app/`이 라우팅·페이지 조립 권한을 소유하므로 FSD `pages`와 충돌한다. element type에 등록하지 않아 그 레이어 사용을 CI가 거부한다.
...
- 새 앱: `apps/{name}/`에 App Router 구조와 공유 `tsconfig`·`eslint-config` 상속을 배선한다.

### 빌드가 강제하는 불변식

- 아래는 리뷰가 아니라 빌드가 막는 경계다. 코드 생성 후 이 목록으로 자기검증한다. 강제 장치(공유 config)가 아직 배선되지 않은 레포에서는 이 자기검증이 유일한 게이트다. 도구 설정·버전은 → [code-quality](code-quality.md).
  ...
  - 서버/클라 누수(클라 모듈이 `api-client`나 `auth`의 server-only 코어를 import) — `server-only`/`client-only` 포이즌 임포트(빌드 에러). `auth`의 클라 훅 엔트리 사용은 정상이다.
