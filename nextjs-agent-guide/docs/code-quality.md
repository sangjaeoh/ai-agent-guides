...
- 강제 기계는 공유 패키지가 소유한다: `packages/eslint-config`·`packages/tsconfig`·루트 dependency-cruiser 설정·`turbo.json`. 앱·패키지는 이를 상속만 한다.
  - 강제 로직을 앱마다 복제하면 드리프트한다. 한 곳에서 선언하고 워크스페이스가 상속한다. 로컬 명령 = CI 명령(`turbo`가 워크스페이스 그래프에 팬아웃·캐싱).
...
- 게이트: 전 스토리 렌더 스모크(test-runner)와 접근성 검사(addon-a11y·axe)를 품질 게이트 파이프라인에 포함한다.
  - 스토리가 있어도 게이트 밖이면 깨진 채 방치된다. 게이트 합류가 강제의 실체다.
