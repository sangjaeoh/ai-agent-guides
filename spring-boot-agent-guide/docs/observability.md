...
- 전역 핸들러는 미분류 5xx만 ERROR로 남기고, 도메인 예외의 4xx 응답은 로깅하지 않는다.

### 상관 ID

- common-web `RequestIdFilter`가 요청 상관 ID를 소유한다.
  - `X-Request-Id` 요청 헤더가 있으면 수용하고 없으면 생성한다.
  - MDC 키 `requestId`로 싣고 응답 헤더로 반환한다.
...
- 계측은 Micrometer로 한다.
- 메트릭 이름은 점 구분 소문자로 짓고 마지막 세그먼트를 측정 대상으로 한다(`cache.evict.errors`).
...
- 캐시 통계는 저장소 통계 활성으로 충족하고 별도 계측하지 않는다 → [caching](caching.md)의 구성.
...
- 분산 트레이싱(Micrometer Tracing·OpenTelemetry): 단일 프로세스는 MDC 상관 ID로 추적하고, 모듈 추출 시 도입을 재평가한다.
