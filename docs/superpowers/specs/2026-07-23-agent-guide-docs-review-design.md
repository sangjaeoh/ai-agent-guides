...
### 2. (철회) testing.md 참조(→) 표기

계획 단계에서 old/new 쌍을 만들며 재검토한 결과, 괄호 후치 인용 11건(`testing.md:70,119,139,144,156,175,197,199,209,244`, `rendering.md:77`) 전부 로컬 규칙 문장 뒤에 붙는 근거 인용이며 prefix 델리게이션(로컬 내용 없이 "주제는 → 문서를 따른다"만 있는 형태)과 용도가 다름을 확인. testing.md:119·139는 문서 링크가 아니라 같은 문서 내 섹션 포인터(→ 테스트하지 않는 것 / → 결정성)로 cross-doc 참조 표기 문제 자체가 아님. prefix로 강제 변환 시 로컬 규칙 문장이 소실되거나 어색한 복합문이 되는 의미 훼손 위험이 있어 철회. "완전 위임(prefix)"과 "로컬 규칙+근거 인용(parenthetical)"은 서로 다른 용도로 일관되게 쓰인 것으로 재판단.

design-system.md:106(항목 6)만 두 패턴 어디에도 안 맞는 진짜 outlier로 남아 계속 수정 대상.
